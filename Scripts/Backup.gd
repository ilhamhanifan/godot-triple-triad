...
func getOpenSlots():
	...
	for x in boardSlots.size():
		for y in boardSlots.size():
			#print(boardSlotsCards[x][y])
			if !boardSlotsCards[x][y]:# and !isPlayerTurn:
				#print('appended')
...
func checkboard(card):
	#print(boardSlotsCards)
	#print(card.get_node("Template").texture.resource_path)

	for x in boardSlotsCards.size():
		for y in boardSlotsCards.size():
			if card == boardSlotsCards[x][y]:
				print(str(x) + ' // ' + str(y) + 'flips ==>' + str(x) + ' // ' + str(y-1))
				#print(card.get_node("Label_top").text)
				if y-1 >= 0:
					print('CASE A')
